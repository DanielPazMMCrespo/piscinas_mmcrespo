                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                @foreach ($bicoes as $container)
                                    ...
                                    <div class="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-4 shadow-sm space-y-3">
                                        ...
                                        <!-- Calibração da Bomba Doseadora -->
                                        @php
                                            $fatorBomba = (float) ($container->fator_correcao ?? 1.0);
                                            $bombaCalibrada = abs($fatorBomba - 1.0) > 0.01;
                                        @endphp
                                        <div class="flex items-center justify-between text-[11px] py-1 px-2.5 rounded-xl {{ $bombaCalibrada ? 'bg-primary-50/80 dark:bg-primary-950/30 text-primary-700 dark:text-primary-300 border border-primary-200/60 dark:border-primary-800/40' : 'bg-slate-50/60 dark:bg-slate-800/40 text-slate-500 dark:text-slate-400 border border-slate-100 dark:border-slate-800/50' }}">
                                            <span class="flex items-center gap-1.5 font-medium truncate">
                                                <x-filament::icon icon="heroicon-m-bolt" class="w-3.5 h-3.5 {{ $bombaCalibrada ? 'text-primary-500' : 'text-slate-400' }}" />
                                                <span>{{ $container->descricaoBomba() }}</span>
                                            </span>
                                            @if ($bombaCalibrada)
                                                <span class="font-bold text-[10px] px-1.5 py-0.5 rounded bg-primary-100 dark:bg-primary-900/60 text-primary-800 dark:text-primary-200">
                                                    {{ number_format($fatorBomba, 2, ',', '') }}×
                                                </span>
                                            @endif
                                        </div>
                                        ...
                                        <div class="pt-2 border-t border-slate-100 dark:border-slate-800/80 flex items-center justify-between text-xs">
                                            ...
                                            @if (auth()->user()?->hasAnyRole([\App\Constants\UserRole::ADMIN, \App\Constants\UserRole::TECNICO]))
                                                <div class="flex items-center gap-1.5">
                                                    <button 
                                                        type="button"
                                                        wire:click="mountAction('calibrarBomba', { container: {{ $container->id }} })"
                                                        title="Calibrar Bomba / Ajustar Potenciómetro"
                                                        class="inline-flex items-center gap-1 px-2.5 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700 font-semibold text-xs transition-colors min-h-[44px]">
                                                        <x-filament::icon icon="heroicon-o-adjustments-horizontal" class="w-4 h-4" />
                                                        <span>Bomba</span>
                                                    </button>
                                                    <button 
                                                        type="button"
