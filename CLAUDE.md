...
23. **Um canal de notificação que atira leva o trabalho de negócio com ele.** `Notification::send($colecao, $n)` percorre destinatários e canais em série e deixa a excepção subir: em 2026-09 o Resend recusou os destinatários (domínio `mmcrespo.pt` por verificar) e isso rebentou com o `ProcessDailyRecordAfterCreate` — as 3 tentativas do job repetiram o desconto de stock e as fotos, e nunca chegaram à torneira, que corria a seguir à notificação. Enviar sempre por `App\Support\NotificacaoResiliente::enviar()` a partir de jobs, comandos, serviços e observers: envia destinatário a destinatário, audita a falha e nunca a propaga. O `via()` de todas as notificações põe `'database'` primeiro e `'mail'` por último, de propósito — o sino do painel fica escrito mesmo quando o e-mail rebenta a seguir; não trocar essa ordem.
24. **Um job com `tries > 1` tem de ser idempotente em tudo o que escreve.** O `handle()` corre inteiro em cada tentativa. O `ProcessDailyRecordAfterCreate` debitava o mesmo consumo 3x e duplicava as `record_photos` porque uma etapa posterior atirava. A marca é `daily_records.stock_processado_em`, lida e escrita dentro da mesma transação com `lockForUpdate()`; as fotos usam `firstOrCreate`. Etapas que só notificam vão para o fim.
25. **`Notification::send` antes de marcar `notified_at`/`sent_at`/cooldown queima o episódio.** Vários sítios (`DosingContainer::notificarSeBaixo`, `HannaCloudSync::notificarThresholds`, `SendComplianceDigestCommand`) marcam o estado *antes* de enviar: uma excepção no envio perdia o alerta para sempre, sem ninguém saber. Com o `NotificacaoResiliente` isto deixa de acontecer, mas a ordem continua a ser: marcar, enviar, nunca ao contrário sem pensar no que se perde.
26. **Numa página com tabela, os modais das ações são impressos pela vista da tabela, não pela da página.** `x-filament-panels::page` só imprime `<x-filament-actions::modals />` quando a página **não** é `HasTable`; nos restantes casos quem os imprime é o fim de `{{ $this->table }}`. Numa vista própria com separadores em Alpine, se a tabela estiver dentro de um `x-show`, o modal herda o `display:none` do contentor: o botão monta a ação no servidor (os testes de `callAction` passam) e **não abre nada** no ecrã. Foi o que aconteceu ao "Ação Técnica" do Registo Diário e ao "Reabastecer" dos bidões no Stock. Numa vista própria com tabela dentro de separadores, imprimir `<x-filament-actions::modals />` no topo da página, antes da tabela — as flags `has*ModalRendered` tornam a segunda impressão inócua, logo não há modais duplicados.
...
- [ ] **`MAIL_FROM_ADDRESS` num domínio verificado no Resend.** Com o remetente de teste (`onboarding@resend.dev`) o Resend só aceita `daniel.paz@mmcrespo.pt` e recusa todos os outros destinatários — foi isso que rebentou com o processamento dos registos diários em 2026-09.
