...
- `remember` (lembrar-me) tem **default `true`** — login persistente ativo por omissão.

## Coisas a rever
- `remember` por defeito `true` é atípico para uma app administrativa/regulamentar (normalmente seria `false` por segurança) — confirmar se é intencional.
