                        ...
                        $poolsByBombas->map(function (Pool $pool) use ($installation, $modoRapido, $poolsByFiltros, $bombasSchema, $tanquesSchema, $lavagemSchema, $enxaguamentoSchema, $posicaoNormalSchema, $nsSchema, $observacoesSchema) {
                            ...
                            if (! self::isNS() && ! $modoRapido) {
                                ...
                                if ($poolsByFiltros->contains('id', $pool->id)) {
                                    // Os data-attributes são o alvo da barra global de timers
                                    // (mmcTimerBar em app.js) para saltar ao separador e à
                                    // secção onde o timer foi iniciado.
                                    $sections[] = Forms\Components\Section::make('Lavagem filtros')
                                        ->extraAttributes(['data-pools-section' => "lavagem-{$pool->id}"])
                                        ->schema($lavagemSchema($pool));

                                    // O caminho é relativo: a secção vive dentro do Tab, que já
                                    // tem statePath "pools.{id}" — um caminho absoluto aqui
                                    // resolveria para pools.{id}.pools.{id}.* e a secção nunca
                                    // apareceria.
                                    $sections[] = Forms\Components\Section::make('Enxaguamento')
                                        ->extraAttributes(['data-pools-section' => "enxaguamento-{$pool->id}"])
