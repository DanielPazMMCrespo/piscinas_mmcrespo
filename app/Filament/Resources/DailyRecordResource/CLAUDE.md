...
## Estrutura de dados
- **Um separador (`Tabs\Tab`) por piscina**, com as secções todas dentro do separador: `Bombas e contadores → Tanques (se a instalação tem `tanques_verificaveis`) → Lavagem filtros → Enxaguamento → Posição normal → Análises → Químicos e Observações`. Antes era um Wizard com um passo por tema e um fieldset por piscina em cada passo — obrigava a atravessar 7 passos para fechar uma piscina. Os campos globais (`hora_colheita`, `ns_foto`) estão na secção "Dados Globais", acima dos separadores. NS só vê "Análises"; modo rápido (`?quick=1&pool=X`) vê "Registo Rápido" + "Químicos e Observações".
- Os separadores levam `data-pools-fieldset={pool_id}` e as secções de filtro `data-pools-section=lavagem|enxaguamento-{pool_id}` — é por aí que a barra global de timers (`mmcTimerBar` em `resources/js/app.js`) troca de separador e faz scroll até ao timer.
- Comparação com a sonda: além do hint, os campos com semáforo mostram no sufixo o delta face à sonda (`↑`/`↓` + valor, vermelho quando divergente). Campos com semáforo e `pressao_filtro` são `live(onBlur: true)` — em 4G o `live(debounce)` disparava um roundtrip por dígito.
